...
@Author: CybeRazor
@Title: getsource_iter.py
@Date: 25/07/2019
@License: GPLv3

@Description:
Fork from the precedent getsource.py script, to execute it recursively for each file in
subdirectories using os.walk(), so instead of using execpy.sh, which works only on bash, you
can use this directly on Python, making everything cross-platform

By default added as '-w' arguments: 'require' and 'include', just for convenience

@Usage:
$ python getsource_iter.py  [-s, --server SERVER_URL/IP] [-w, --word-fetch WORD_TO_FETCH] [-f, --file-fetch PATH/TO/FILE]
OR
$ chmod +x getsource_iter.py
$ ./getsource_iter.py  [-s, --server SERVER_URL/IP] [-w, --word-fetch WORD_TO_FETCH] [-f, --file-fetch PATH/TO/FILE]
...
def error(err):
	...
	sys.stderr.write('\nUsage: python getsource_iter.py  [-s, --server SERVER_URL/IP] [-w, --word-fetch WORD_TO_FETCH] [-f, --file-fetch PATH/TO/FILE]\n')
	sys.exit(1)
...
if __name__ == '__main__':
	...
	for x in range(10):
		...
		for cwd, dirs, files in os.walk(os.getcwd()):
			...
			for f in files:
				# execute the main function passing argv + 'include' and 'require' as '-w' args + filepath as '-f' arg
				main(sys.argv[1:] + ['-w', 'include', '-w', 'require', '-f', os.path.join(cwd, f)])
